...
- **D. 热点模式（LocalOnlyHotspot，已评估、M1 不做）**：由发送端开"仅本地热点"、
  接收端用 WifiNetworkSpecifier 加入，可彻底绕开路由器 AP 隔离/双频隔离，上层代码零改动。
  技术可行（Android 官方 API，无需 root），但主人决定 M1 先不做，保留为 M2 候选。
  代价：两端都会失去外网（单芯片手机不支持 STA+AP 并发）。若真机上 ICE 直连失败，这是首选解法。
