 ...
 * 发现阶段的一问一答（走 TCP）。
 *
 * **为什么不用 UDP 广播**：广播是"喊"，对方可能听不见 —— 路由器开 AP 隔离、
 * 热点过滤组播、换网卡后 socket 还绑在旧接口上。而扫端口是"敲门"，
 * **能敲开就是能敲开**，而且完全不需要 Wi-Fi 扫描权限。
 *
 * 两个端口各司其职：接收端在 [PORT] 这个**固定端口**上待着，
 * 应答里带回真正的**信令端口**，后者才是跑投屏协商的那个。
 *
